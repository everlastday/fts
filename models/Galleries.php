...
class Galleries extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            ['url', 'unique', 'targetClass' => 'app\models\Galleries', 'message' => 'Цей адрес для галереї уже занятий.'],
        ];
    }
	...
	public static function showCategories($categories_id = array()) {
		...
		if(!empty($categories_id and is_array($categories_id))) {
			...
			foreach ($categories_id as $category_id) {
				...
				if(!empty($categories[$category_id]['category_name'])) {
					$data .= "<li>" . $categories[$category_id]['category_name'] . '</li>';
				}
			}
